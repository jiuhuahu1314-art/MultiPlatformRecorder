...
#pragma warning restore VSTHRD100 // Avoid async void methods
        {
            if (CommandArgumentFirstRun)
            {
                _ = Task.Run(async () =>
                {
                    ...
                    {
                        MessageBox.Show(Window.GetWindow(this), @"星辰欲梦 安装成功！
...
You can uninstall me in system settings.", "安装成功 Installed", MessageBoxButton.OK, MessageBoxImage.Information);
            ...
            while (true)
            {
                try
                {
                    ...
                    try
                    {
                        if (config.Global.WpfAutoStart)
                            AutoStartHelper.SetEnabled(true);
                        config.Global.PropertyChanged += (_, e) =>
                        {
                            if (e.PropertyName == nameof(config.Global.WpfAutoStart))
                                AutoStartHelper.SetEnabled(config.Global.WpfAutoStart);
                        };
                    }
                    catch { }
