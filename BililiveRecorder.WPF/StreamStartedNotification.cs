    ...
    internal static class StreamStartedNotification
    {
        ...
        internal class NotificationCenterApi : INotificationApi
        {
            ...
            public async Task ShowAsync(IRoom room)
            {
                ...
                var roomUrlStr = (room.RoomConfig.Platform != "bilibili" && !string.IsNullOrEmpty(room.RoomConfig.RoomUrl))
                    ? room.RoomConfig.RoomUrl
                    : "https://live.bilibili.com/" + room.RoomConfig.RoomId;
                var roomUrl = new Uri(roomUrlStr);
                var builder = new ToastContentBuilder()
                    .AddHeader("BililiveRecorder-StreamStarted", "星辰欲梦开播通知", "")
